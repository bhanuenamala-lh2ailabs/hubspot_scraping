...
def build_message(to_addr, subject, body, attachments=(), sender=None, html=None, cc=None):
...
def send(to_addr, subject, body, attachments=(), dry=False, html=None, cc=None):
    ...
    try:
        if t == "gmail_oauth":
            r = _gmail_send(_oauth_creds(), build_message(to_addr, subject, body, attachments, html=html, cc=cc))
            return t, r.get("id", "sent")
        if t == "gmail_sa":
            r = _gmail_send(_sa_creds(),
                            build_message(to_addr, subject, body, attachments, sender=IMPERSONATE, html=html, cc=cc))
            return t, r.get("id", "sent")
        if t == "smtp":
            user = env["smtp_user"]
            msg = build_message(to_addr, subject, body, attachments, sender=user, cc=cc)
