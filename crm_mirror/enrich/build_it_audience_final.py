# -*- coding: utf-8 -*-
"""FINAL IT-services LinkedIn audience: 113 net-new + STRICT-salvage of DEAD IT deals (all owners).
NO live/in-pipeline deals. Claude reads each dead deal's call note -> keep only real re-engage signals."""
...
def salvage(company, note):
    prompt=(f"IT-services firm '{company}' was cold-called for a CODEBASE-ACQUISITION deal and marked DEAD. "
            f"Call note: \"{note[:600]}\"\nIs there a genuine signal it's worth ONE soft LinkedIn re-touch "
            "(wrong timing, busy/call-back-later, wrong person but referral, soft/price rejection, conditional "
            "interest)? NO if hard dead-end (scam, firmly not interested, defunct, wrong-number, no product). "
            "Return ONLY JSON {\"salvage\":true|false}.")
...
# ALL IT deals -> keep DEAD only
after=None; deals=[]
while True:
    b={"limit":100,"properties":["dealname","dealstage"],"filterGroups":[{"filters":[{"propertyName":"scraped_type","operator":"EQ","value":"ITservices"}]}]}
    if after: b["after"]=after
    s,d=call("/crm/v3/objects/deals/search","POST",b); deals+=d.get("results",[]); after=d.get("paging",{}).get("next",{}).get("after")
    if not after: break
    time.sleep(0.03)
dead=[x for x in deals if stage.get(x["properties"].get("dealstage"),"").startswith("Dead")]
print(f"IT dead deals (all owners): {len(dead)} (live excluded)", flush=True)
d2c={}
for i in range(0,len(dead),100):
    s,d=call("/crm/v4/associations/deals/contacts/batch/read","POST",{"inputs":[{"id":x["id"]} for x in dead[i:i+100]]})
...
for x in dead:
    ...
    if not (comp and (em or (fn and ln))): continue
    s,a=call(f"/crm/v4/objects/deals/{x['id']}/associations/notes")
    nids=[str(t["toObjectId"]) for t in a.get("results",[])]; body=""
    if nids:
    ...
    checked+=1
    if body.strip() and salvage(comp, clean(body)):
        salv.append({"email":em,"firstname":fn,"lastname":ln,"jobtitle":clean(c.get("jobtitle")) or "Founder","employeecompany":comp,"country":"IN","googleaid":""})
    if checked%30==0: print(f"  reviewed {checked}/{len(dead)} -> salvaged {len(salv)}", flush=True)
    time.sleep(0.1)
print(f"strict-salvaged dead: {len(salv)}", flush=True)
...
print(f"\nFINAL IT AUDIENCE = {len(out)} (net-new {len(netnew)} + strict-salvaged dead {len(salv)}) | with_email={sum(1 for r in out if r['email'])} | NO live deals", flush=True)
