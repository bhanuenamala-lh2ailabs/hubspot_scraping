...
TO = "bhanu.enamala@lh2.ai"   # current-week-only variant, default recipient per standing mail rule
...
def main():
    ...
    weeks = weeks[-1:]     # THIS-WEEK-ONLY variant: keep just the current, partial week's column.
                           # widx() below still only matches days that fall inside this single
                           # range, so every counter naturally excludes prior weeks' activity.
...
By furthest stage = each engaged lead once, at the deepest stage it reached ('Touched, screened
    ...
    out = os.path.join(HUB, "godown", "weekly_reports", "wow_metrics_thisweek.html")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    open(out, "w", encoding="utf-8").write("<!doctype html><meta charset=utf-8>" + HTML)
    print(f"\nwrote {out}")
    if not SEND:
        print(f"DRY — re-run with --send to mail {TO}"); return
    import gmail_sender
    t, det = gmail_sender.send(TO, f"LH2 supply funnel — this week so far ({span})", text, html=HTML)
