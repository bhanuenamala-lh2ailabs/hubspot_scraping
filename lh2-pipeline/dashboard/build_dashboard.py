...
def call(path, method="GET", payload=None):
    ...
    # A full build is ~1,000 sequential calls over several minutes, so a single dropped
    # connection anywhere in that window used to abort the whole run: there was no
    # `except Exception`, so URLError / SSLError / socket timeout escaped uncaught. That is
    # what killed six local builds on 6 Aug while short one-off calls to the same host
    # succeeded 3/3. Transport errors are now retried like any other transient, and 5xx is
    # treated as transient alongside 429.
    for attempt in range(6):
        try:
            ...
            if attempt == 5:
                raise
            print(f"   transport retry {attempt+1}/6 ({type(e).__name__})", file=sys.stderr)
...
# has to be explicit.
# The GTM-analyst / lead-manager / lead-closer split was removed on 2026-08-14: everyone now
# works the whole funnel, so these are simply the company's metrics and every one of them is
# shown for every person. The grouping comments below are kept ONLY to explain what each metric
# means — they no longer imply a role, a panel or a target.
METRICS = {
    # --- calling ---
...
# ---- NOTE-ONLY ACTIVITY -------------------------------------------------------------------
# A second call-back that rang out moves no stage and satisfies no KPI, so under stage metrics
# alone it never happened. The 6:30pm mail has counted these buckets for a while; the dashboard
# now shows the same ones.
#
# The rules are IMPORTED, never copied. An earlier hand-copy of the KPI definitions drifted and
# inflated dial counts 2.7x, and these classifiers are the same kind of thing: one regex edited
# in one place and not the other silently changes what a bucket means.
#
# note_rules.py sits NEXT TO this file, inside the repo, on purpose. The first version of this
# import reached across into crm_mirror/enrich/ — which is not tracked in this repo — so it
# resolved on the laptop and raised ImportError on GitHub Actions, where the note KPIs would
# then have been empty on every deployed build with only a stderr line to show for it. A plain
# import with no try/except is deliberate: if this file cannot be found, the build SHOULD die
# loudly rather than publish a dashboard that quietly lost a panel.
from note_rules import NOTE_RULES, classify as _classify_note, plain as _plain_note

# Buckets that merely restate a stage KPI are mapped away, exactly as the mailer does it, so the
# same act is never counted once as a KPI and again as note-only activity.
NOTE_DROP = {"System note", "Script received"}


def note_buckets():
    """-> {deal_id: [(day, actor_owner_id, bucket), ...]} for every deal, via batch reads.

    Batched deliberately: one association call per deal would be ~1,700 requests on top of the
    ~1,700 the history pass already makes, roughly doubling a build that already takes minutes.
    The v4 batch endpoint does 100 deals a call.
    """
    ids = [did for _, did in funnel]
    d2n, allnotes = {}, set()
    for i in range(0, len(ids), 100):
        ...
        for r in (a.get("results") or []):
            ...
            if fid and ns:
                d2n[fid] = ns; allnotes.update(ns)
        if (i // 100) % 5 == 0:
            print(f"   note assocs {min(i+100, len(ids))}/{len(ids)}", file=sys.stderr)
    print(f"   {len(allnotes)} notes on {len(d2n)} deals — reading bodies", file=sys.stderr)
    meta = {}
    nl = sorted(allnotes)
    ...
    for did, ns in d2n.items():
        ...
        for n in ns:
            p = meta.get(n)
            if not p: continue
            body = _plain_note(p.get("hs_note_body") or "")
            if not body.strip(): continue
            b = _classify_note(body)
            if b in NOTE_DROP: continue
            got.append([ist_day(p.get("hs_timestamp")), p.get("hubspot_owner_id") or "", b])
        if got: out[did] = got
    return out


print("fetching note activity for every deal (batched)...", file=sys.stderr)
_nb = note_buckets()
_idx_by_id = {did: idx for idx, did in funnel}
for _did, _evs in _nb.items():
    _i = _idx_by_id.get(_did)
    if _i is None: continue
    rows[_i]["nb"] = _evs          # [[day, owner_id, bucket], ...]
print(f"   note activity attached to {len(_nb)} deals", file=sys.stderr)
