# -*- coding: utf-8 -*-
"""Push the 63 fully-qualified India startups (30-500 employees, Indian founder, engineering
team confirmed) 50:50 Lamiya/Ishpreet. Final re-dedup against HubSpot right before writing.
Domain conflicts auto-attach the contact to the existing deal instead of failing.

Usage: python3 push.py [--apply]
"""
...
rows = json.load(open("/tmp/india_startups_fully_qualified.json"))
ready = []
for r in rows:
...
for x in push:
    ...
    dp = {"dealname": x.get("Company") or f'{x.get("First Name")} {x.get("Last Name")}',
          ...
          "lead_source": SOURCE, "source_tab": "india_startups_ops",
          "lh2_domain": dom, "linkedin_url": (x.get("LinkedIn URL") or "").strip(),
          "description": f'{x.get("First Name")} {x.get("Last Name")} ({x.get("Title","")}) '
                          f'{x.get("employeeCount","")} employees; via India Startups Ops sourcing; {x.get("_e164")}'}
