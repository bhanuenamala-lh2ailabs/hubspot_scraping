# -*- coding: utf-8 -*-
"""SignalHire-then-Apollo enrichment for the refreshed distressed-startups qualified list.
NOTE: Apollo is at 0 credits as of today (confirmed via a direct 422 CREDITS_EXHAUSTED test) —
the fallback calls below will no-op until the account is topped up; this run is effectively
SignalHire-only.
"""
...
IN_JSON = "/tmp/distressed2_qualified.json"
OUT_CSV = os.path.join(HUB, "temporary", "Distressed_Startups2_qualified_enriched.csv")
STATE = os.path.join(os.path.dirname(__file__), "distressed2_enrich_state.json")
