# -*- coding: utf-8 -*-
"""Flatten the Stage 1 (people) and Stage 2 (company) Apify raw JSON into clean CSVs, joined
by company LinkedIn URL, plus a failed_urls.csv for anything that didn't come through.
"""
import json, csv, os, re

HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "apify_out")

people = json.load(open(os.path.join(OUT, "people_raw.json")))
companies = json.load(open(os.path.join(OUT, "companies_raw.json")))
input_company_urls = json.load(open("/tmp/gujarat_company_urls.json"))
input_people = json.load(open("/tmp/gujarat_pushed_subset.json"))
...
for p in people:
    ...
    company_url_by_person[p.get("linkedinUrl", "").strip().lower().rstrip("/")] = cu
    people_rows.append({
...
# ---------------------------------------------------------------- join: add company data onto people rows
...
for pr in people_rows:
    ...
    joined = dict(pr)
    for k in ("employeeCount", "foundedYear", "industries", "companyType", "website", "hqCity", "hqCountry"):
...
failed_people = [r for r in input_people if nurl(r.get("LinkedIn URL")) not in scraped_people_urls]
...
with open(os.path.join(OUT, "failed_urls.csv"), "w", newline="", encoding="utf-8") as f:
    ...
    for r in failed_people:
        w.writerow(["person", r.get("LinkedIn URL"), f'{r.get("First Name")} {r.get("Last Name")}'])
...
print(f"\ndata quality: {no_founded}/{len(company_rows)} companies missing foundedYear, "
