# -*- coding: utf-8 -*-
"""Push the final Kolkata Sales Nav ready-list, 50:50 Yuktha/Lamiya. Same shape as the
Gujarat/Jaipur/Coimbatore/MP pushes: final re-dedup against HubSpot right before writing,
lead_source stays "LinkedIn Sales Nav ( IT Services )", source_tab="kolkata_salesnav", contact
website set from the company's email domain where derivable. Domain conflicts auto-attach the
contact to the existing deal instead of failing.

Usage: python3 kolkata_push.py [--apply]
"""
...
import csv
enriched = list(csv.DictReader(open(os.path.join(HUB, "temporary", "Kolkata_IT_qualified_enriched.csv"),
                                     encoding="utf-8")))
