# -*- coding: utf-8 -*-
"""Title-filter + dedup pass for the refreshed India Distressed Startups export (campaign-tool
format — extra Lead/Connection Status/etc columns, same First/Last/Headline/Location/Company/
Title/LinkedIn URL fields we need). Same keep-list as the first batch: founder/CEO/CTO/CPO/owner
only. Dedupes against local prequal-reserve + current HubSpot state (now clean, since the first
batch was just fully removed) before spending SignalHire/Apollo credits.

Output: /tmp/distressed2_qualified.json
"""
...
IN_CSV = os.path.join(HUB, "temporary", "India_Distressed_Startups_SalesNav (1).csv")
OUT_JSON = "/tmp/distressed2_qualified.json"
