# -*- coding: utf-8 -*-
"""Flatten the Kolkata Apify raw JSON (people + companies) into clean CSVs joined by company
LinkedIn URL, plus a failed_urls.csv for anything that didn't come through.
"""
import json, csv, os

HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "apify_out_kolkata")

people = json.load(open(os.path.join(OUT, "people_raw.json")))
companies = json.load(open(os.path.join(OUT, "companies_raw.json")))
input_company_urls = json.load(open("/tmp/kolkata_company_urls.json"))
input_people = json.load(open("/tmp/kolkata_qualified.json"))
...
failed_people = [r for r in input_people if r.get("LinkedIn URL") and nurl(r.get("LinkedIn URL")) not in scraped_people_urls]
...
with open(os.path.join(OUT, "failed_urls.csv"), "w", newline="", encoding="utf-8") as f:
    ...
    for r in failed_people:
        w.writerow(["person", r.get("LinkedIn URL"), r.get("Company")])
