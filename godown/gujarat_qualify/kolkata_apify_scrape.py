# -*- coding: utf-8 -*-
"""Two-stage Apify LinkedIn scrape for the Kolkata Sales Nav qualified list: profile scrape to
find each person's current-role company LinkedIn URL, then company scrape on the deduped set —
same pattern as the Gujarat/MP wrong-fit screening pilots.
"""
...
OUT_DIR = os.path.join(HUB, "godown", "gujarat_qualify", "apify_out_kolkata")
os.makedirs(OUT_DIR, exist_ok=True)

rows = json.load(open("/tmp/kolkata_qualified.json"))
...
json.dump(urls, open("/tmp/kolkata_profile_urls.json", "w"))
...
print(f"people with an extractable company URL: {len(people_items)-no_url}/{len(people_items)} "
      f"({100*(len(people_items)-no_url)/max(len(people_items),1):.1f}%)")
...
json.dump(dedup, open("/tmp/kolkata_company_urls.json", "w"))
