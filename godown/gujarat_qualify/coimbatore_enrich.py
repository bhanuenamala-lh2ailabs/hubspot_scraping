# -*- coding: utf-8 -*-
"""SignalHire-then-Apollo enrichment for the Coimbatore Sales Nav qualified list.

Same SignalHire reveal-by-LinkedIn-URL as the Gujarat batch. Anyone SignalHire leaves without
a valid +91 mobile falls back to Apollo people/match (by person name + company name, since
there's no domain on this sheet) for one more shot at a phone/email.
"""
...
IN_JSON = "/tmp/coimbatore_qualified.json"
OUT_CSV = os.path.join(HUB, "temporary", "Coimbatore_IT_qualified_enriched.csv")
STATE = os.path.join(os.path.dirname(__file__), "coimbatore_enrich_state.json")
...
results = []
sh_hit = apollo_used = apollo_hit = 0
for i, r in enumerate(rows, 1):
    ...
    if key in state:
        ...
        # Apollo fallback only if SignalHire found no dialable +91 mobile
        ...
        time.sleep(0.25)
    if s.get("email") or s.get("phone"): sh_hit += 1
    r2 = dict(r)
