...
enriched = list(csv.DictReader(open(os.path.join(HUB, "temporary", "Distressed_Startups2_qualified_enriched.csv"),
                                     encoding="utf-8")))
...
for x in push:
    ...
    dp = {"dealname": x.get("Company") or f'{x.get("First Name")} {x.get("Last Name")}',
          ...
          "lead_source": SOURCE, "source_tab": "distressed_startups_salesnav2",
          "lh2_domain": dom, "linkedin_url": (x.get("LinkedIn URL") or "").strip(),
          "description": f'{x.get("First Name")} {x.get("Last Name")} ({x.get("Title","")}) '
                          f'via India Distressed Startups LinkedIn Sales Nav (refresh); {x.get("_e164")}'}
